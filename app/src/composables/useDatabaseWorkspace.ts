...
type QueryRow = Record<string, string | number | boolean>

type DatabaseSnippetItem = {
  id: string
  name: string
  category: string
  commands: string
...
function normalizeEngine(value: string, fallback: DbEngine = 'MySQL'): DbEngine | null {
...
function mapTable(item: any): DatabaseTable {
  return {
...
function mapConnection(item: any): DatabaseConnection {
...
function normalizeQueryRows(rows: any[]): QueryRow[] {
...
function downloadSpreadsheetFallback(name: string, columns: string[], rows: QueryRow[]) {
...
function getDefaultQueryTemplate(engine: DbEngine) {
  return DEFAULT_QUERY_TEMPLATE[engine] || DEFAULT_QUERY_TEMPLATE.MySQL
}

function normalizeConnectionPayload(payload: Partial<DatabaseConnectionPayload> | null | undefined, existing?: DatabaseConnection | null) {
...
export function useDatabaseWorkspace(params: UseDatabaseWorkspaceParams) {
  ...
  const databaseStateLabel = (state: DbState) => {
    if (state === 'connected') return '已连接'
    if (state === 'offline') return '不可达'
    return '待连接'
  }

  const databaseStateClass = (state: DbState) => {
